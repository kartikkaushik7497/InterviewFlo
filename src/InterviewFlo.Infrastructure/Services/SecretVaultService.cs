...
internal sealed class SecretVaultService : ISecretVaultService
{
    ...
    {
        var folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "InterviewFlo");
    ...
    {
        var master = Environment.GetEnvironmentVariable("INTERVIEWFLO_MASTER_SECRET");
        var material = $"InterviewFlo|{Environment.UserName}|{Environment.MachineName}|{master}";
